...
def crear_funcion_trigger(cursor, tabla):
    ...
    CREATE OR REPLACE FUNCTION audit_{tabla}_after_insert()
    RETURNS TRIGGER AS $$
    BEGIN
        INSERT INTO auditoria (nombre_tabla, usuario_db, accion, descripcion_cambios)
        VALUES ('{tabla}', current_user, 'INSERT', json_build_object({insert_values}));
        RETURN NEW;
    END;
    $$ LANGUAGE plpgsql;
    ...
    FOR EACH ROW
    EXECUTE PROCEDURE audit_{tabla}_after_insert();
    """

    funcion_update_sql = f"""
    CREATE OR REPLACE FUNCTION audit_{tabla}_after_update()
    RETURNS TRIGGER AS $$
    BEGIN
        INSERT INTO auditoria (nombre_tabla, usuario_db, accion, descripcion_cambios)
        VALUES ('{tabla}', current_user, 'UPDATE', json_build_object({update_values}));
        RETURN NEW;
    END;
    $$ LANGUAGE plpgsql;
    ...
    FOR EACH ROW
    EXECUTE PROCEDURE audit_{tabla}_after_update();
    """

    funcion_delete_sql = f"""
    CREATE OR REPLACE FUNCTION audit_{tabla}_after_delete()
    RETURNS TRIGGER AS $$
    BEGIN
        INSERT INTO auditoria (nombre_tabla, usuario_db, accion, descripcion_cambios)
        VALUES ('{tabla}', current_user, 'DELETE', json_build_object({delete_values}));
        RETURN OLD;
    END;
    $$ LANGUAGE plpgsql;
    ...
    FOR EACH ROW
    EXECUTE PROCEDURE audit_{tabla}_after_delete();
    """
...
def generar_pdf(logs, usuario, fecha_inicio, fecha_fin):
    pdf_filename = f'reporte_logs_{usuario}{fecha_inicio.replace(":", "-").replace(" ", "")}_a{fecha_fin.replace(":", "-").replace(" ", "_")}.pdf'
    ...
    title = f"Reporte: Logs de PostgreSQL\nUsuario: {usuario}\nDesde: {fecha_inicio}\nHasta: {fecha_fin}"
        ...
        if os.path.getsize(log_path) > 0:
            with open(log_path, 'rb') as file:
                ...
                for line in decoded_data.splitlines():
                    match = re.match(
                        r"(?P<fecha_hora>\d{6}\s+\d{2}:\d{2}:\d{2})\s+\d+\s+\w+\s+\w+",
                        line
                    )
                    if match:
...
def opcion2(cursor):
    ...
    log_directory = r"/var/log/postgresql"  # Ajustar la ruta al directorio de logs de PostgreSQL
